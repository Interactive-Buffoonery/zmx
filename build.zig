...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == .macos) {
        exe_mod.addCSourceFile(.{ .file = b.path("src/darwin_proc.c"), .flags = &.{} });
    }
    ...
    // Test
    {
        ...
        if (target.result.os.tag == .macos) {
            test_module.addCSourceFile(.{ .file = b.path("src/darwin_proc.c"), .flags = &.{} });
        }
    ...
    // Release step - cross-compile to all targets from any host
    {
        ...
        for (release_targets) |release_target| {
            ...
            if (resolved.result.os.tag == .macos) {
                release_mod.addCSourceFile(.{ .file = b.path("src/darwin_proc.c"), .flags = &.{} });
            }
