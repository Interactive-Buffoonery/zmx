...
fn exec(sesh_name: []const u8, cmd: Cmd) !noreturn {
    ...
    // environment: Zig's Init environment view remains live until shutdown.
    var kept_count: usize = 0;
    var current = std.c.environ;
    while (current[0]) |entry| : (current += 1) {
        const value = std.mem.span(entry);
        if (std.mem.startsWith(u8, value, "AMX_STATUS_FILE=") or
            std.mem.startsWith(u8, value, "AMX_STATUS_TOKEN=")) continue;
        kept_count += 1;
    }

    const envp = try gpa.allocSentinel(?[*:0]const u8, kept_count + 1, null);
    var index: usize = 0;
    current = std.c.environ;
    while (current[0]) |entry| : (current += 1) {
        const value = std.mem.span(entry);
        if (std.mem.startsWith(u8, value, "AMX_STATUS_FILE=") or
            std.mem.startsWith(u8, value, "AMX_STATUS_TOKEN=")) continue;
    ...
    envp[index] = session_env.ptr;

    const err = lib_posix.execvpeZ(cmd.file, cmd.argv_ptr, envp.ptr);
