    ...
    const emitSessionEnd = struct {
        fn call(
            ...
            status.StatusFile.emitSessionEnd(
                ...
                code,
                session,
                @intCast(std.Io.Timestamp.now(event_io, .real).toSeconds()),
            ) catch |err| std.log.warn("failed to emit session-end status: {s}", .{@errorName(err)});
            emitted.* = true;
        }
    }.call;
        ...
        if (poll_fds.items[1].revents & lib_posix.POLL.IN != 0) {
            const n = read_buf.read(client_sock_fd) catch |err| {
                ...
                if (err == error.ConnectionResetByPeer or err == error.BrokenPipe) {
                    emitSessionEnd(gpa, status_cfg, session_name, .unknown, 0, &saw_session_end, io);
                    return ClientResult{ .kind = .detach, .session_name = null };
                }
                ...
                switch (msg.header.tag) {
                    ...
                    .SessionEnd => {
                        if (msg.payload.len == @sizeOf(ipc.SessionEnd)) {
                            const end = std.mem.bytesToValue(ipc.SessionEnd, msg.payload[0..@sizeOf(ipc.SessionEnd)]);
                            emitSessionEnd(gpa, status_cfg, session_name, end.reason, end.code, &saw_session_end, io);
                        } else {
                            emitSessionEnd(gpa, status_cfg, session_name, .unknown, 0, &saw_session_end, io);
                        }
                        return ClientResult{ .kind = .detach, .session_name = null };
...
fn initTerminal(gpa: std.mem.Allocator, io: std.Io, size: ipc.TerminalSize, cfg: *const Cfg) !ghostty_vt.Terminal {
...
fn daemonLoop(daemon: *Daemon, gpa: std.mem.Allocator, io: std.Io, server_sock_fd: lib_posix.socket_t, pty_fd: i32) !void {
    ...
    daemon_loop: while (daemon.running) {
        ...
        if (poll_fds.items[1].revents & inp_flags != 0) {
            ...
            if (n_opt) |n| {
                if (n == 0) {
                    // EOF normally means the PTY child is exiting. Give it a
                    // short non-blocking grace period so we can publish its real
                    // status without hanging forever on a process that closed
                    // stdio but kept running.
                    var wait_result = lib_posix.waitpid(daemon.pid, cross.c.WNOHANG);
                    var reap_polls: u8 = 0;
                    while (wait_result.pid != daemon.pid and reap_polls < 100) : (reap_polls += 1) {
                        std.Io.sleep(io, std.Io.Duration.fromMilliseconds(1), .real) catch {};
                        wait_result = lib_posix.waitpid(daemon.pid, cross.c.WNOHANG);
                    }
                    if (wait_result.pid != daemon.pid) {
                        lib_posix.kill(-daemon.pid, lib_posix.SIG.KILL) catch {};
                        wait_result = lib_posix.waitpid(daemon.pid, 0);
                    }
                    daemon.child_reaped = true;
                    const signal_bits = wait_result.status & 0x7f;
                    const code: i32 = if (signal_bits == 0)
                        @intCast((wait_result.status >> 8) & 0xff)
                    else
                        128 + @as(i32, @intCast(signal_bits));
        ...
        clients_loop: while (i > 0) {
            ...
            if (revents & lib_posix.POLL.IN != 0) {
                ...
                while (client.read_buf.next()) |msg| {
                    switch (msg.header.tag) {
                        .Input => try daemon.handleInput(gpa, client, &term, msg.payload),
                        ...
                        .CwdQuery => try daemon.handleCwd(gpa, client),
                        .Run => try daemon.handleRun(gpa, io, client, msg.payload),
                        .Ack, .TaskComplete, .LabelData, .EnvData, .SessionEnd, .CwdResponse => {},
...
pub const Daemon = struct {
    ...
    pub fn ensureSession(self: *Daemon, io: std.Io) !bool {
        ...
        if (exists) {
            ...
            while (true) {
                if (ipc.probeSession(std.heap.c_allocator, self.socket_path)) |probe| {
                    probe.deinit();
                    if (self.command != null) std.log.warn(
    ...
    fn run(self: *Daemon, io: std.Io, dir: std.Io.Dir, sesh_name: []const u8) !bool {
        ...
        defer {
            ...
            if (!self.child_reaped) _ = lib_posix.waitpid(self.pid, 0);
        }
    ...
    fn broadcastSessionEnd(self: *Daemon, reason: ipc.SessionEndReason, code: i32) void {
        ...
        for (self.clients.items) |client| {
            ipc.send(client.socket_fd, .SessionEnd, std.mem.asBytes(&payload)) catch |err| {
                std.log.warn("failed to send SessionEnd fd={d}: {s}", .{ client.socket_fd, @errorName(err) });
            };
        }
    }
    ...
    pub fn handleInput(
        ...
        if (self.leader_client_fd == client.socket_fd) {
            if (term.flags.mouse_event == .none and util.isMouseReport(payload)) return;
    ...
    pub fn handleResizePixels(self: *Daemon, client: *Client, pty_fd: i32, payload: []const u8) void {
    ...
    pub fn handleDetach(self: *Daemon, gpa: std.mem.Allocator, client: *Client, i: usize) void {
        ...
        const payload = ipc.SessionEnd.init(.detached, 0);
        ipc.send(client.socket_fd, .SessionEnd, std.mem.asBytes(&payload)) catch {};
    ...
    pub fn handleDetachAll(self: *Daemon, gpa: std.mem.Allocator) void {
        ...
        while (self.clients.items.len > 0) {
            ...
            const client_to_close = self.clients.items[last];
            const payload = ipc.SessionEnd.init(.detached, 0);
            ipc.send(client_to_close.socket_fd, .SessionEnd, std.mem.asBytes(&payload)) catch {};
            _ = self.closeClient(gpa, client_to_close, last, false);
        }
    }
    ...
    pub fn handleCwd(self: *Daemon, gpa: std.mem.Allocator, client: *Client) !void {
        const resolved = cwd_mod.cwdForSession(gpa, self.pty_fd, self.pid) catch null;
        defer if (resolved) |path| gpa.free(path);
        const response = if (resolved) |path| ipc.CwdResponse.fromPath(path) else ipc.CwdResponse.empty();
        try ipc.appendMessage(gpa, &client.write_buf, .CwdResponse, std.mem.asBytes(&response));
        client.has_pending_output = true;
    }
...
test "terminal retains the configured scrollback without the default byte cap" {
    ...
    var term = try initTerminal(alloc, std.testing.io, .{
        .cols = 80,
        .rows = 24,
        .xpixel = 0,
        .ypixel = 0,
