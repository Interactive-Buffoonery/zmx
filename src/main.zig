...
pub fn main(init: std.process.Init) !void {
    ...
    const status_cfg = try status.StatusConfig.takeFromEnv(gpa);
    defer status_cfg.deinit(gpa);
    ...
    if (std.mem.eql(u8, cmd, "version") or std.mem.eql(u8, cmd, "v") or std.mem.eql(u8, cmd, "-v") or std.mem.eql(u8, cmd, "--version")) {
        ...
        defer gpa.free(sesh);
        return cwdCommand(gpa, io, &cfg, sesh);
        ...
        return attach(gpa, io, &daemon, status_cfg, parsed.labels);
...
fn help(io: std.Io) !void {
    const help_text =
        ...
        \\  AMX_STATUS_FILE      Append-only status event file used by awesoMux
...
fn cwdCommand(alloc: std.mem.Allocator, io: std.Io, cfg: *Cfg, session_name: []const u8) !void {
    const socket_path = try socket.getSocketPath(alloc, cfg.socket_dir, session_name);
    defer alloc.free(socket_path);
    const payload = try ipc.roundTripForTag(alloc, socket_path, .CwdQuery, "", .CwdResponse);
    defer alloc.free(payload);
    if (payload.len != @sizeOf(ipc.CwdResponse)) return error.CwdResponseProtocolMismatch;
    const response = std.mem.bytesToValue(ipc.CwdResponse, payload[0..@sizeOf(ipc.CwdResponse)]);
    if (response.overflow != 0) return error.CwdResponseOverflow;
...
fn attach(
    ...
    const looper = try loop.clientLoop(client_sock, status_cfg, daemon.session_name, io);
    switch (looper.kind) {
        .detach => return,
        .switch_session => {
            if (looper.session_name) |session_name| {
                ...
                return attach(gpa, io, &target_daemon, status_cfg, null);
            }
        },
    }
}
